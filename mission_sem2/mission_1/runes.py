...
def _TYPE(types):
    ...
    def decorator(func):
        name = func.__name__
        def type_checked(*args):
            act = len(args)
            if act != exp:
                raise Exception(f"""in Function {name}
  Expected {exp} number of arguments
  but given {act} number of arguments""")
            for i in range(exp):
                if not isinstance(args[i], types[i]):
                    raise TypeError(f"""in Function {name}
  Expected type "{types[i].__name__}" for argument #{i + 1}
  but given {args[i]} of type "{type(args[i]).__name__}" instead""")
            return func(*args)
        return type_checked
    return decorator
...
def hollusion(rune, ports=None):
    ...
    if ports == None or ports <= 2:
        num = 9
    buffers = list(map(lambda p: graphics.open_pixmap("buffer", viewport_size, viewport_size),
                       range(num)))
